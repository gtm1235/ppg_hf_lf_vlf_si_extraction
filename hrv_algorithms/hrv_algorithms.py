...
def process_rr_nn_intervals(si_np):
    ...
    si_df_no_outliers = remove_outliers(rr_intervals=si_np, low_rri=400, high_rri=2000, verbose=False)
    ...
    si_df_nn_intervals_list = remove_ectopic_beats(rr_intervals=si_df_interpolated_rr_intervals, method="malik", verbose=False)
    si_df_interpolated_nn_intervals = interpolate_nan_values(rr_intervals=si_df_nn_intervals_list)
